...
namespace AppConstants {
    ...
    const QString DEFAULT_STYLE =
        ...
        "QTextBrowser { background-color: #1e1e1e; color: #e0e0e0; border: 1px solid #333; border-radius: 4px; padding: 5px; }"
    ...
    const QString LIGHT_STYLE =
        ...
        "QTextBrowser { background-color: white; color: #202020; border: 1px solid #ccc; border-radius: 4px; padding: 5px; }"
...
// -----------------------------
// Text Processing Helpers (unchanged)
// -----------------------------
class TextProcessor {
public:
...
// -----------------------------
// Knowledge Base Manager (unchanged)
// -----------------------------
class KnowledgeBase {
public:
...
// -----------------------------
// Enhanced Chat Window with Improved GUI
// -----------------------------
...
public:
    ChatWindow(QWidget *parent = nullptr) 
        ...
        // Use a stronger encryption key for this build.
        const std::string kbFile = "knowledge_base.dat";
        const std::string encryptionKey = "k1eFjP@7xL9qZ#5mR2tY8sA3vB6nC0wD";
        m_knowledgeBase = std::make_shared<KnowledgeBase>(kbFile, encryptionKey);
...
private slots:
    ...
    void onOpenBrowser() {
        ...
        connect(browserWin, &BrowserWindow::pageScraped, this, [this](const QString &htmlContent) {
            conversationDisplay->append("<i>Scraped page content received. Processing...</i>");
        });
        browserWin->setAttribute(Qt::WA_DeleteOnClose);
        browserWin->show();
    }
    ...
    void onLoadDataFromWeb() {
        ...
        QUrl url("https://raw.githubusercontent.com/AbiramiSBN/ai-model/refs/heads/main/knowledge_base.dat");
        QNetworkRequest request(url);
    ...
    void onLoadJSONData() {
        ...
        if (count > 0)
            conversationDisplay->append(formatInfoMessage("Loaded " + QString::number(count) + " entries from JSON file."));
    }
    ...
    void onAbout() {
        QMessageBox::about(this, "About " + AppConstants::APP_NAME,
            ...
            "<p>Features include local encrypted storage, web and JSON data loading, dark/light themes, "
            "command processing, text-to-speech, and conversation logging.</p>"
        );
    }
...
private:
    ...
    void setupUI() {
        // Create a main splitter to separate conversation display and input panel
        QSplitter *mainSplitter = new QSplitter(Qt::Vertical, this);
        
        // Upper widget: Conversation display in a QTextBrowser
        ...
        // Lower widget: Input area with line edit and buttons
        ...
        sendButton->setToolTip("Send message (Enter)");
        loadWebButton = new QPushButton("Load Web Data", this);
        loadWebButton->setToolTip("Load additional data from the web");
        loadJSONButton = new QPushButton("Load JSON", this);
        loadJSONButton->setToolTip("Load training data from a JSON file");
        trainButton = new QPushButton("Train", this);
        trainButton->setToolTip("Train from DuckDuckGo data");
        voiceButton = new QPushButton("Voice", this);
        voiceButton->setToolTip("Read last bot message aloud");
        ...
        mainSplitter->addWidget(conversationDisplay);
    ...
    void setupMenus() {
        QMenu *browserMenu = menuBar()->addMenu("&Browser");
        QAction *openBrowserAction = browserMenu->addAction("Open Built-in Browser");
        connect(openBrowserAction, &QAction::triggered, this, &ChatWindow::onOpenBrowser);
    ...
    void setupToolBar() {
        ...
        QAction *toggleThemeAction = toolBar->addAction(QIcon::fromTheme("preferences-desktop-theme"), "Toggle Theme");
        toggleThemeAction->setToolTip("Toggle between dark and light theme");
        connect(toggleThemeAction, &QAction::triggered, this, &ChatWindow::onToggleTheme);
        
        QAction *clearAction = toolBar->addAction(QIcon::fromTheme("edit-clear"), "Clear Conversation");
        clearAction->setToolTip("Clear the conversation display");
        connect(clearAction, &QAction::triggered, this, &ChatWindow::onClearConversation);
    }
